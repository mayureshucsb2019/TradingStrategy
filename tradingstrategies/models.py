...
class OrderResponse(BaseModel):
    ...
    price: Optional[float] = Field(
        None, example=14.21, title="Order Price"
    )  # Will be null if type is MARKET
    quantity_filled: float = Field(..., example=10, title="Quantity Filled")
    vwap: Optional[float] = Field(
        None, example=14.21, title="Volume-Weighted Avg Price"
    )
    status: Literal["OPEN", "TRANSACTED", "CANCELLED"] = Field(
        ..., example="OPEN", title="Order Status"
    )
...
class OHLCParams(BaseModel):
    ...
    period: Optional[int] = Field(
        None,
        title="Period to retrieve data from",
        description="Defaults to current period if not specified",
    )
    limit: Optional[int] = Field(
        20, title="Limit of results", description="Defaults to 20 if not specified"
    )
...
class TimeSalesParams(BaseModel):
    ...
    after: Optional[int] = Field(
        None,
        title="ID after which to retrieve data",
        description="Retrieve data with an ID value greater than this",
    )
    period: Optional[int] = Field(
        None,
        title="Period to retrieve data from",
...
class OrdersParams(BaseModel):
    ...
    status: Optional[OrderStatus] = Field(
        OrderStatus.OPEN,
        title="Order Status",
        description="Filter orders by status. Defaults to 'OPEN'.",
    )
